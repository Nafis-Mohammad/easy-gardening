...
def home(request):
    ...
    maxQuanProd = Product.objects.filter(
        quantity_sold=maxQuant['quantity_sold__max'])
...
def product_search(request):
    ...
    if query:
        products = Product.objects.filter(
            product_name__icontains=query).order_by('-price')
...
def add_to_wishlist(request):
    ...
    #     user=customer)
    wishItem, created = WishlistItem.objects.get_or_create(
        customer=customer, wishproduct=product)
...
def wishlist_view(request):
    ...
    #products = Product.objects.filter(id=wishlist_items)
    #context = {'wishlist_items': wishlist_items, 'products': products}
